...
const GLOSSARY = {
  ...
  "Ht":   { tex:"H^{\\,t}", name:"拓扑分支表示",
           formula:"H^{(\\ell+1)}=\\alpha\\,\\hat A\\,H^{(\\ell)}+H_0^{\\,t}",
           desc:"在 $\\hat A$ 上做 $L$ 步 APPNP 迭代。\n越深层综合越多邻居信息，$\\alpha$ 控制扩散强度。",
           role:"分支输出" },
  "Ha":   { tex:"H^{\\,a}", name:"属性分支表示",
           formula:"H^{(\\ell+1)}=\\alpha\\,\\hat S\\,H^{(\\ell)}+H_0^{\\,a}",
           desc:"同一迭代，底图换成 $\\hat S$。\n结构上无连接但属性接近的节点也能被聚到一起。",
           role:"分支输出" },
