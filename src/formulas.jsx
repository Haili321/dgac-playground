...
const GLOSSARY = {
  ...
  "Lemma1Neu":{ tex:"\\text{Lemma 1}", name:"Neumann 级数 · Neumann series",
           formula:"(I-M)^{-1}=\\textstyle\\sum_{\\ell=0}^\\infty M^\\ell,\\quad |\\lambda_{\\max}(M)|<1",
           desc:"论文 Lemma 1（Sec. 3.3）—— 经典线代恒等式：主导特征值 $|\\lambda_{\\max}(M)|<1$ 时，$I-M$ 可逆且逆矩阵可以展开为 $M^\\ell$ 的几何级数和。\n\nDGAC 里的应用形式（把 $M$ 换成 $\\alpha M$）：\n$(I-\\alpha M)^{-1}=\\sum_{\\ell=0}^\\infty \\alpha^\\ell M^\\ell$，需要 $\\alpha\\,|\\lambda_{\\max}(M)|<1$。\n对 $\\tilde A, \\hat A, \\tilde S$（特征值 $\\in[-1,1]$），$\\alpha<1$ 即可收敛。\n\n用途：把 Eq.10/11/15 的闭式解（矩阵求逆）变成 Eq.12/13/16 的迭代扩散形式，可以用 $L$ 步截断近似。\n这是 DGAC 「把优化问题变成扩散迭代」的核心数学工具。",
           role:"paper 数学工具" },
