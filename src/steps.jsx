...
const STEPS = [
  ...
  },
  {
    ...
    active: ["kmeans", "cprop"],
    formula: "cprop",
    desc: "DGAC 的点睛之笔：把硬分配 C₀ 也放回图上扩散 —— 错误分配的节点被邻居「纠正」，形成软分配 C。不需要梯度，靠扩散本身平滑。系数是 γ，与分支扩散的 α 区分开（playground toy 实现共用一个滑块）。剧场里的微妙之处：protagonist 被 km 错分的根本原因通常不是邻居们 km 也错了，而是它有跨簇邻居（异质边）—— 这些邻居本身 km 是对的，但在特征扩散时把 k-means 的簇心往自己那边拉偏了。cprop 能把它纠回来，靠的是同簇邻居数 > 跨簇邻居数（论文的 homophily 假设）；异质图上这条件破坏，DGAC 就用属性分支 Ŝ 补救。",
  },
  {
    id: "loss",
    title: "自监督目标",
